...
import {View, Dimensions, Image, Text, Slider, TouchableWithoutFeedback, TouchableOpacity, ActivityIndicator, StyleSheet} from 'react-native';
import Video from 'react-native-video';
import Orientation from 'react-native-orientation';
import { Toast, Provider, Icon } from "@ant-design/react-native";
...
export default class VideoPlayScreen extends Component {
          ...
          <View style={{ width: this.state.videoWidth, height: this.state.videoHeight, backgroundColor:'#000000' }}>
            ...
            </TouchableWithoutFeedback>
            {
              this.state.showVideoControl ?
                <View style={[styles.controlWrapper, {height: this.state.videoHeight}]}>
                  <View style={[styles.controlTop, {width: this.state.videoWidth}]}>
                    <TouchableOpacity activeOpacity={0.3} onPress={() => { this._getBack() }}>
                      <Image
                        style={styles.playControl}
                        source={require('../../assets/image/icon_video_back.png')}
                      />
                    </TouchableOpacity>
                  </View>
  ...
  _onPlayError = () => {
    Toast.offline('视频加载失败，即将跳转回首页', 1, () => {
      this.props.navigation.goBack()
    });
    console.log('视频播放失败');
  };
  ...
  onControlShrinkPress() {
    if (this.state.isFullScreen) {
      this.props.store.updateFullScreen()
