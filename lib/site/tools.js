...
const DOMParser = require('beautiful-dom');
...
const zy = {
  ...
  film_get_type_zero(txt, key) {
    return new Promise((resolve, reject) => {
      try {
        const html = new DOMParser(txt);
        const list = html.querySelectorAll('.xing_vb li');
        const d = {list: [], total: 0, update: 0};
        const url = getSite(key).url;
        for (let i = 1; i < list.length - 31; i++) {
          const info = {
            site: key,
            name: list[i].querySelector('.xing_vb4 a').innerText,
            type: list[i].querySelector('.xing_vb5').innerText,
            time: list[i].querySelector('.xing_vb7')
              ? list[i].querySelector('.xing_vb7').innerText
              : '',
            detail: url + list[i].querySelector('a').getAttribute('href'),
            index: 0,
          };
          d.list.push(info);
        }
        d.update = parseInt(
          html.querySelectorAll('.xing_top_right li strong')[0].innerText,
        );
      ...
      try {
        ...
        for (let i = 0; i < list.length; i++) {
          const info = {
            ...
            detail:
              url + list[i].querySelector('.address').getAttribute('href'),
            index: 0,
          };
          d.list.push(info);
        }
        d.update = parseInt(
          html.querySelectorAll('.header_list li span')[0].innerText,
        );
      ...
      try {
        ...
        for (let i = 1; i < list.length - 1; i++) {
          const info = {
            ...
            detail:
              url +
              list[i].childNodes[1].querySelector('a').getAttribute('href'),
            index: 0,
          };
          d.list.push(info);
        }
  ...
  detail_get_type_zero(txt, key) {
    return new Promise((resolve, reject) => {
      try {
        ...
        data.imgUrl = html.querySelector('.lazy').outerHTML;
        console.log(2.5);
